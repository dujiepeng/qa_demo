...
typedef MessagesViewItemLongPressPositionHandler =
    List<ChatUIKitEventAction>? Function(
      BuildContext context,
      MessageModel model,
      Rect rect,
...
typedef MessagesViewMorePressHandler =
    List<ChatUIKitEventAction>? Function(
      BuildContext context,
      List<ChatUIKitEventAction> defaultActions,
    );
...
typedef CreateGroupHandler =
    Future<CreateGroupInfo?> Function(
      BuildContext context,
...
typedef ChatUIKitNewRequestItemBuilder =
...
typedef ChatUIKitMoreActionsBuilder<T> =
    List<ChatUIKitEventAction<T>> Function(
      BuildContext context,
      List<ChatUIKitEventAction<T>> actions,
    );

/// 时间格式化
typedef TimeFormatterHandler =
...
typedef ChatUIKitAppBarActionsBuilder =
    List<ChatUIKitAppBarAction>? Function(
      BuildContext context,
      List<ChatUIKitAppBarAction>? defaultList,
    );

/// 用于在联系人详情页添加 list item，会返回默认的列表，需要你调整后返回来，返回来的数据会用于菜单显示。
typedef ChatUIKitDetailItemBuilder =
...
enum ChatUIKitMessageLongPressMenuStyle { popupMenu, bottomSheet }

enum ChatUIKitMessageAttachmentMenuStyle { menu, bottomSheet }
