...
class ConversationListViewController
  ...
  ConversationListViewController({this.willShowHandler}) {
  ...
  void onMessageContentChanged(
    ...
    if (index != -1) {
      list.cast<ConversationItemModel>()[index] = list
          .cast<ConversationItemModel>()[index]
  ...
  Future<void> markConversationAsRead(String conversationId) async {
    try {
      ...
      debugPrint('conversation list markConversationAsRead: $e');
    }
  }
  ...
  Future<void> fetchItemList({bool force = false}) async {
    ...
    try {
      ...
      List<ConversationItemModel> tmp = await mapperToConversationModelItems(
        items,
      );
      list.clear();
      list.addAll(tmp);
  ...
  Future<List<Conversation>> clearEmptyAndChatRoomConversations(
    ...
    // 获取配置值，判断是否允许空会话
    bool enableEmpty =
        ChatUIKit.instance.options?.enableEmptyConversation ?? false;
  ...
  Future<List<Conversation>> fetchConversations() async {
    ...
    if (!hasFetchPinned) {
      try {
        CursorResult<Conversation> result = await ChatUIKit.instance
            .fetchPinnedConversations(pageSize: 50);
    ...
    try {
      CursorResult<Conversation> result = await ChatUIKit.instance
          .fetchConversations(pageSize: pageSize, cursor: cursor);
  ...
  Future<List<ConversationItemModel>> mapperToConversationModelItems(
    ...
    Map<String, ChatUIKitProfile> profilesMap = ChatUIKitProvider.instance
        .getProfiles(tmp);
