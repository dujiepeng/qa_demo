...
class MessagesViewController extends ChangeNotifier
  ...
  void fetchItemList() async {
    ...
    if (searchedMsg != null && hasSearched == false) {
      List<Message> searchList = await ChatUIKit.instance
      ...
      List<Message> beforeSearchList = await ChatUIKit.instance
          .loadLocalMessages(
            conversationId: profile.id,
            type: conversationType,
            count: pageSize,
  ...
  void onMessageContentChanged(
    ...
    final index = msgModelList.indexWhere(
      (element) => element.message.msgId == message.msgId,
    );
    if (index != -1) {
  ...
  void onMessageReactionDidChange(List<MessageReactionEvent> events) async {
    ...
    for (var reactionEvent in events) {
      if (reactionEvent.conversationId == profile.id) {
        ...
        if (index != -1) {
          Message? msg = await ChatUIKit.instance.loadMessage(
            messageId: msgModelList[index].message.msgId,
          );
          if (msg != null) {
            needUpdate = true;
            List<MessageReaction>? reactions = await msg.reactionList();
  ...
  void onMessageSendSuccess(String msgId, Message msg) {
    final index = msgModelList.indexWhere(
      (element) =>
          element.message.msgId == msgId &&
          msg.status != element.message.status,
    );
  ...
  Future<void> editMessage(Message message, String content) async {
    ...
    try {
      ...
      final index = msgModelList.indexWhere(
        (element) => msg.msgId == element.message.msgId,
      );
      if (index != -1) {
        msgModelList[index] = msgModelList[index].copyWith(message: msg);
        refresh();
      ...
      if (url != null) {
        try {
          ChatUIKitPreviewObj? obj = await ChatUIKitURLHelper().fetchPreview(
            url,
          );
          msg.addPreview(obj);
          ChatUIKit.instance.updateMessage(message: msg);
  ...
  Future<void> recallMessage(Message message) async {
    int index = msgModelList.indexWhere(
      (element) => message.msgId == element.message.msgId,
    );
    if (index != -1) {
      try {
  ...
  Future<void> sendImageMessage(
    ...
    final ImageStream stream = Image.file(
      file,
  ...
  Future<void> sendMessage(Message message) async {
    ...
    if (url == null) {
      final msg = await ChatSDKService.instance.sendMessage(
        message: willSendMsg,
      );
  ...
  Future<void> sendConversationsReadAck() async {
    ...
    if (conversationType == ConversationType.Chat) {
      try {
        ...
        if (unreadCount > 0) {
          await ChatUIKit.instance.sendConversationReadAck(
            conversationId: profile.id,
          );
          for (var element in msgModelList) {
            element.message.hasReadAck = true;
  ...
  Future<void> deleteMessages({required List<String> messageIds}) async {
    try {
      ...
      msgModelList.removeWhere(
        (element) => messageIds.contains(element.message.msgId),
      );
  ...
  Future<void> unpinMessage(Message message) async {
    try {
      ...
      int index = pinedMessagesList.indexWhere(
  ...
  Future<void> fetchPinnedMessages() async {
    try {
      List<Message> ret = await ChatUIKit.instance.fetchPinnedMessages(
        conversationId: profile.id,
      );
