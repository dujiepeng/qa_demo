...
class ContactListViewController with ChatUIKitListViewControllerBase {
  ContactListViewController({this.willShowHandler, this.enableRefresh = true});
  ...
  @override
  Future<void> fetchItemList({bool force = false, bool reload = false}) async {
    if (reload) {
  ...
  void addUser(String userId) {
    if (list.cast<ContactItemModel>().any(
      (element) => element.profile.id == userId,
    )) {
      return;
    }
