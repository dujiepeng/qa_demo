...
class ThreadsView extends StatefulWidget {
  ThreadsView.arguments(ThreadsViewArguments arguments, {super.key})
...
class _ThreadsViewState extends State<ThreadsView>
  ...
  void fetchThreads() async {
    ...
    fetching = true;
    try {
      CursorResult<ChatThread> result = await ChatUIKit.instance
      ...
      Map<String, Message> map = await ChatUIKit.instance
      ...
      for (var key in map.keys) {
        ...
        if (index != -1) {
          list[index] = list[index].copyWith(lastMessage: map[key]);
        }
      }
      ...
      if (mounted) {
        ChatUIKitRoute.pushOrPushNamed(
          ...
          ThreadMessagesViewArguments(
            controller: ThreadMessagesViewController(model: value),
