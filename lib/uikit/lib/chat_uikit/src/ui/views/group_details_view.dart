...
import '../../../../chat_sdk_context/src/context.dart';
import '../../../../chat_uikit.dart';

class GroupDetailsView extends StatefulWidget {
  GroupDetailsView.arguments(GroupDetailsViewArguments arguments, {super.key})
...
class _GroupDetailsViewState extends State<GroupDetailsView>
  ...
  void fetchGroupInfos() {
    isNotDisturb.value = ChatSDKContext.instance.conversationIsMute(
      profile!.id,
    );
    fetchGroup();
  }
  ...
  Widget _buildContent() {
    ...
    models.add(
      ChatUIKitDetailsListViewItemModel(
        title: ChatUIKitLocal.groupDetailViewClearChatHistory.localString(
          context,
        ),
    ...
    content = ListView(children: [content, ...list]);
    ...
    if (group?.permissionType == GroupPermissionType.Owner) {
      list.add(
        ChatUIKitEventAction.normal(
          ...
          label: ChatUIKitLocal.groupDetailViewTransferGroup.localString(
            context,
          ),
          onTap: () async {
            Navigator.of(context).pop();
      ...
      list.add(
        ChatUIKitEventAction.destructive(
          ...
          label: ChatUIKitLocal.groupDetailViewDisbandGroup.localString(
            context,
          ),
          onTap: () async {
            Navigator.of(context).pop();
  ...
  void destroyGroup() async {
    final ret = await showChatUIKitDialog(
      title: ChatUIKitLocal.groupDetailViewDisbandAlertTitle.localString(
        context,
      ),
      content: ChatUIKitLocal.groupDetailViewDisbandAlertSubTitle.localString(
        context,
      ),
      context: context,
      actionItems: [
    ...
    if (ret == true) {
      ChatUIKit.instance
          .destroyGroup(groupId: profile!.id)
          .then((value) {
            if (mounted) {
              ChatUIKitRoute.popToGroupsView(
                context,
  ...
  void leaveGroup() async {
    final ret = await showChatUIKitDialog(
      ...
      content: ChatUIKitLocal.groupDetailViewLeaveAlertSubTitle.localString(
        context,
      ),
      context: context,
      actionItems: [
