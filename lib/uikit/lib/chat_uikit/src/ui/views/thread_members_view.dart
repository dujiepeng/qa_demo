...
class ThreadMembersView extends StatefulWidget {
  ThreadMembersView.arguments(ThreadMembersViewArguments arguments, {super.key})
...
class _ThreadMembersViewState extends State<ThreadMembersView>
  ...
  void initState() {
    ...
    controller =
        widget.controller ?? ThreadMembersViewController(thread: widget.thread);
  ...
  void onProfilesUpdate(Map<String, ChatUIKitProfile> map, [String? belongId]) {
    ...
    if (controller.modelsList.any(
      (element) => map.keys.contains(element.profile.id),
    )) {
