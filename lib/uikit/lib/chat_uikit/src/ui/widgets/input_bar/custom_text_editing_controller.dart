...
class CustomTextEditingController extends TextEditingController {
  ...
  CustomTextEditingController({String? text, this.mentionStyle})
    : super(text: text);
  ...
  void at(ChatUIKitProfile profile) {
    ...
    value = TextEditingValue(
      text:
          text.substring(0, value.selection.baseOffset) +
          addText +
  ...
  void addText(String newStr) {
    ...
    if (currentSelection.baseOffset != currentSelection.extentOffset) {
      ...
      if (currentSelection.start < 0) {
        ...
        str =
            text.substring(0, currentSelection.start) +
            newStr +
  ...
  void deleteTextOnCursor() {
    ...
    if (currentSelection.baseOffset != currentSelection.extentOffset) {
      String subText = text.replaceRange(
        currentSelection.start,
        currentSelection.end,
        '',
      );
      ...
      value = TextEditingValue(text: subText, selection: newSelection);
    } else if (currentSelection.baseOffset == 0 &&
  ...
  TextEditingValue mentionFilter(TextEditingValue newValue) {
    final currentCount = newValue.text
        .split('')
