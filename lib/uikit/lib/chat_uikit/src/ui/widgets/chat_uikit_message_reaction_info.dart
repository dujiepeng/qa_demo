...
class _ChatUIKitMessageReactionInfoState
  ...
  void onReactionDeleteTap(MessageReaction reaction) {
    ...
    int index = reactions.indexWhere(
      (element) => reaction.reaction == element.reaction,
    );
...
class _ChatReactionInfoWidgetState extends State<ChatReactionInfoWidget>
  ...
  void fetchReactionInfo() async {
    ...
    fetching = true;
    try {
      CursorResult<MessageReaction> result = await ChatUIKit.instance
          .fetchReactionDetail(
      ...
      Map<String, ChatUIKitProfile> map = ChatUIKitProvider.instance
          .getProfiles(
            reaction.userList
                .map((e) => ChatUIKitProfile.contact(id: e))
                .toList(),
          );
