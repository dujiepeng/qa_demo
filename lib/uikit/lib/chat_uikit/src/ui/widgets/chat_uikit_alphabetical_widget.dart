...
class _ChatUIKitAlphabeticalWidgetState
  ...
  Widget themeBuilder(BuildContext context, ChatUIKitTheme theme) {
    if (!widget.enableSorting || widget.list.isEmpty) {
      return widget.builder.call(context, widget.list);
    }

    Widget content = widget.builder.call(context, sortList());
  ...
  Widget letterWidget(double height) {
    ...
    content = Container(color: Colors.transparent, width: 100, child: content);
  ...
  List<ChatUIKitListItemModelBase> sortList() {
    ...
    List<NeedAlphabetical> tmp = widget.list
        .whereType<NeedAlphabetical>()
        .toList();
    ...
    for (var item in tmp) {
      String? letter = ChatUIKitAlphabetSortHelper.instance.sortHandler?.call(
        widget.groupId,
        item.profile.id,
        item.showName,
      );
    ...
    for (var item in targetList) {
      ...
      final letterModel =
          widget.itemModelBuilder?.call(item) ??
