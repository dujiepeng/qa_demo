...
enum InputType { normal, text, emoji }
...
class ChatRoomInputBarState extends State<ChatRoomInputBar>
        ...
        children: [
          ...
          Expanded(
            child: Padding(
              ...
              child: Text(
                widget.inputHint ??
                    ChatUIKitLocal.roomInputHint.localString(context),
                maxLines: 1,
  ...
  Widget normalWidget() {
    ...
    CornerRadius radius = ChatUIKitSettings.inputBarRadius;
    ...
    content = Row(textDirection: widget.textDirection, children: children);
        ...
        child: ChatInputEmoji(
          deleteOnTap: () {
            ...
            if (selection.baseOffset != selection.extentOffset) {
              ...
              String text =
                  subText.characters.skipLast(1).toString() +
  ...
  TextSpan buildTextSpan({
    ...
    if (includeEmojis.isNotEmpty) {
      ...
      for (var item in includeEmojis) {
        if (item.index > index) {
          tp.add(
            TextSpan(text: text.substring(index, item.index), style: style),
          );
        }
        tp.add(
          WidgetSpan(
      ...
      if (index < text.length) {
        tp.add(
          TextSpan(text: text.substring(index, text.length), style: style),
        );
      }
    } else {
      tp.add(TextSpan(text: text, style: style));
    }
...
class EmojiIndex {
  ...
  int length;
  String emoji;
  EmojiIndex({required this.index, required this.length, required this.emoji});
}
