...
class _BlockListViewState extends State<BlockListView>
  ...
  void onProfilesUpdate(Map<String, ChatUIKitProfile> map, [String? belongId]) {
    ...
    if (controller.list.any(
      (element) => map.keys.contains((element as ContactItemModel).profile.id),
    )) {
      for (var element in map.keys) {
        int index = controller.list.indexWhere(
          (e) => (e as ContactItemModel).profile.id == element,
        );
        if (index != -1) {
        ...
        return ChatUIKitListView(
          ...
          itemBuilder: (context, model) {
            if (model is ContactItemModel) {
              ...
              item = SizedBox(key: ValueKey(model.profile.id), child: item);
