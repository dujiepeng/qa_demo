...
typedef ChatUIKitWidgetBuilder =
...
class ChatUIKitRouteBackModel {
  ...
  const ChatUIKitRouteBackModel({required this.type, required this.profileId});

  /// 用于复制当前对象。 [ChatUIKitRouteBackModel] 为不可变对象，当需要修改时，需要复制一份新的对象。
  ChatUIKitRouteBackModel copy() {
    return ChatUIKitRouteBackModel(type: type, profileId: profileId);
  }
...
class ChatUIKitRoute {
  ...
  final Map<String, ChatUIKitWidgetBuilder>
  uikitRoutes = <String, ChatUIKitWidgetBuilder>{
    ...
    ChatUIKitRouteNames.searchView: (context, arguments) {
      return SearchView.arguments(arguments as SearchViewArguments);
    },
    ...
    ChatUIKitRouteNames.threadsView: (context, arguments) {
      return ThreadsView.arguments(arguments as ThreadsViewArguments);
    },
  ...
  /// 返回到根页面
  static popToRoot(BuildContext context, {ChatUIKitRouteBackModel? model}) {
  ...
  static Future<T?> pushOrPushNamed<T extends Object?>(
    ...
    if (hasInit) {
      return Navigator.of(context).pushNamed(pushName, arguments: arguments);
    } else {
      return Navigator.of(context).push(
        MaterialPageRoute(
          builder: (context) {
            return ChatUIKitRouteNames.getWidgetFromName(pushName, arguments);
          },
        ),
      );
    }
